...
def main():
	...
	USAGE
	  ImageScript.py [flags] function args
	...
	EXAMPLES
	  ImageScript.py F2P ../Images smileyimage?
	  ImageScript.py cropwhite ./whiteborderimg.png
...
## Converts FITS images in subdirectories of argv[2] to PNG format
  ...
  ## argv[4] changes colourmap option
def FITStoPNG(args):
	
  params=""
  namext=""
  ## Change colourmap option
  if len(args)>4:
		if args[4]=="BB":
			params=" -cmap BB "
			namext="_cBB"
  ...
  for f_img in file_seek(args[2], args[3]+"*.fits"):
		...
		## Outfile name
		p_img = outdir+"/"+os.path.basename(f_img)[:-5]+namext+".png"
		
		## File conversion
		os.system("ds9 "+f_img+" "+params+" -minmax -colorbar no -saveimage png "+p_img+" -exit")
		i+=1
...
## Crop white pixels from image border
	## Expects a pixel-array with 255-pixels around central image
	## Rough version
def pngcropwhite1(imgarr, level=254.9):
	imgarr = imgarr[imgarr!=255]
	dim = int(numpy.sqrt(imgarr.size))
	return numpy.reshape(imgarr,[dim,dim])
